...
# Transfer bootstrap values where available
valid <- which(from_idx >= 1 & from_idx <= length(iq_bs) &
                 to_idx >= 1 & to_idx <= length(tt_bs))
tt_bs[to_idx[valid]] <- iq_bs[from_idx[valid]]
...
# Save a new timetree WITH bootstrap labels
dir.create("treetime_L", showWarnings = FALSE)  # ensure folder exists

out_with_bs <- file.path(getwd(), "treetime_L", "timetree_with_bootstrap.nexus")
write.nexus(t_tt, file = out_with_bs)
...
## Read input data
tree     <- read.nexus("treetime_L/timetree_with_bootstrap.nexus")
metadata <- read_csv("../metadata.filtered.csv", show_col_types = FALSE)
...
# Read the NEW time-scaled tree that has bootstrap labels
tree_bs <- read.nexus(file.path(getwd(), "treetime_L", "timetree_with_bootstrap.nexus"))
...
p_region_hm <- ggplot(heatmap_data, aes(x = "Region", y = y, fill = Region)) +
  ...
  scale_fill_manual(
    ...
    values = c(
      "East Africa"     = "#1B9E77",
      "West Africa"     = "#7570B3",
      "Southern Africa" = "#8C7853",
      "Central Africa"  = "#E6AB02",
      "North Africa"    = "#C2B280"
    )
  ) +
  theme_void() +
...
## Save final outputs
ggsave("L_combined_time_tree_regions.pdf",
       plot = combined_plot, width = 16, height = 10,
       units = "in", limitsize = FALSE)

ggsave("L_combined_time_tree_regions.png",
       plot = combined_plot, width = 16, height = 10,
       units = "in", dpi = 600, bg = "white")
...
# Save a new timetree WITH bootstrap labels
dir.create("treetime_S", showWarnings = FALSE)  # ensure folder exists

out_with_bs <- file.path(getwd(), "treetime_S", "timetree_with_bootstrap.nexus")
write.nexus(t_tt, file = out_with_bs)
...
# Read the NEW time-scaled tree that has bootstrap labels
tree_bs <- read.nexus(file.path(getwd(), "treetime_S", "timetree_with_bootstrap.nexus"))

# Base tree with bootstrap
p_host_base <- suppressWarnings(ggtree(tree_bs, mrsd = mrsd) + theme_tree2())
...
ggsave("S_combined_time_tree_regions.pdf",
       plot = combined_plot, width = 16, height = 10,
       units = "in", limitsize = FALSE)

ggsave("S_combined_time_tree_regions.png",
       plot = combined_plot, width = 16, height = 10,
       units = "in", dpi = 600, bg = "white")
